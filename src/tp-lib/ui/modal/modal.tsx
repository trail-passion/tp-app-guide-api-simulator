...
export interface ErrorOptions extends ModalOptions {
    /** Default to `title` or "OK" if undefined. */
    labelOK?: string
}
...
export default class Modal extends AbstractModal {
    ...
    protected actualShow(): boolean {
        ...
        if (React.isValidElement(content)) {
            ...
            createRoot(frame).render(JSON.stringify(content))
        }
    ...
    static async error(content: ModalContent | ErrorOptions): Promise<void> {
        const options = normalizeContent(content)
        return new Promise((resolve) => {
            const hide = () => {
                modal.hide()
 ...
 * ```
 */
function getHumanFriendlyErrorContent(
    content: unknown
):
    | JSX.Element
